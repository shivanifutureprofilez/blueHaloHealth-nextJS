  ...
  const [list, setLists] = useState([]);
   const fetchSubServiceDetails = async () => {
    try {
      ...
      const response = await main.singleSubServiceDetail(pageID);
      if (response.data) {
        const service = response?.data?.subServiceData;
...
const handleUpdate = async () => {
  try {
    ...
    // MUST include ID!
    const response =  lists.updateSubService(pageID, formdata);
